import arch from './data/images.json'
...
function init() {
  ...
  } else if (meta.portsHint && el.portsHint) {
    el.portsHint.textContent = meta.portsHint
  }
  renderLayers()
  renderFlows()
  bindEvents()
...
function renderFlows() {
  ...
  const defaultFlow = state.data.meta?.defaultFlowId
  if (defaultFlow) selectFlow(defaultFlow)
}
...
function selectFlow(flowId) {
  ...
  if (flow) {
    ...
    if (pathLabels.length) appendMeta('Path', pathLabels.join(' → '))
    if (flow.example) appendMeta('Example', flow.example)
...
async function initDiagrams() {
  const container = document.getElementById('diagram-cards')
  const diagrams = state.data.diagrams
...
init()
initDiagrams().catch((err) => console.warn('Mermaid diagrams:', err))
